...
export class StaySuggestionPageComponent implements OnInit {
  ...
  filterApartments(): void {
    ...
    fetch(`https://apartmain.azurewebsites.net/api/apartments/filter`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
        Accept: 'application/json',
  ...
  rateStay(): void {
    ...
    fetch(
      `https://apartmain.azurewebsites.net/api/staybookings/verifyowner?bookingNumber=${this.bookingNumber}&bookingPIN=${this.bookingPin}`,
      {
        method: 'GET',
      }
    )
